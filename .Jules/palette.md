...
## 2026-03-24 - Focus visible styles required for native inputs without Tailwind Forms

**Learning:** Without `@tailwindcss/forms` plugin installed, native HTML inputs (like `<input type="checkbox">` and `<input type="radio">`) do not get automated, distinct focus rings on keyboard navigation without polluting mouse clicks.
**Action:** Always add explicit `focus-visible` classes (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1`) directly to the HTML class attributes for checkboxes and radios to ensure robust keyboard accessibility. Do not inject them via JS if purge mechanisms are at risk.
