...
## 2026-03-13 - Explicit Focus States for Native Form Controls
**Learning:** When using Tailwind CSS without the `@tailwindcss/forms` plugin, native `<input type="checkbox">` and `<input type="radio">` elements lack sufficient visual focus indicators by default, significantly degrading keyboard accessibility.
**Action:** Always apply explicit focus classes (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1`) directly to these elements in the HTML to ensure they are fully navigable via keyboard, while avoiding `focus:` to prevent disruptive focus rings on mouse click.
