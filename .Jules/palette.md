...
## 2024-11-20 - Accessible Dynamic Notifications
**Learning:** Dynamic elements like custom toast notifications and loading spinners are visually apparent but often completely invisible to screen readers without specific roles and live region attributes.
**Action:** When creating custom notifications, always add `aria-live="polite"` and `aria-atomic="true"` to the container. Add `role="status"` to loading indicators and `aria-hidden="true"` to their decorative animated icons to ensure assistive technologies announce status changes cleanly.
